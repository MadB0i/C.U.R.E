...
| 1.2b | Scan detects shortcut | PASS | `SAFE … CURE_TEST_startup.lnk (startup-folder) id=c7262bc7cbfc174a`, T1547.001. |
| 1.2c | Scan task row | SKIPPED | Correctly absent (creation SKIPPED above). |
| 1.2d | Risk expectation | PASS | Both rows Safe — correct: the helper is signed-notepad bytes, so detection ≠ flagging. No HIGH anywhere (summary `117 entries, 0 high-risk`). |
...
1. **F-LNK-1 (minor, display-only):** scan shows the `.lnk` target as relative garbage (`..\..\..\..\..\..\..\Local\Temp\… [MISSING]`) while Windows resolves the same shortcut to the correct absolute path. The lnk parser appears to prefer/resolve the embedded relative path incorrectly. Quarantine is unaffected (moves by location). UNVERIFIED whether all shortcuts or only WScript-created ones.
