...
Counts: **PASS 34 · FAIL 0 · SKIPPED 5** (task-create, task-row, overlay, live-uninstall-in-§1.5, plus restore-point self-query) **· UNVERIFIED 0** (everything not run is marked SKIPPED with reason, nothing claimed).
...
2. **F-LOLBIN-1 — FIXED (this session, commits `eca1712`/`0031665`/`add1c66`/`599e205`/`c24bcb5`):** signed LOLBins with suspicious args scored `0 Safe` (`-20 trusted` + `-40 Valid` cancelling `+25`/`+30`). Now discounts are withheld for the 9 known hosts when a command-line heuristic fires, `%VAR%` expands across the whole command, and a LOLBin-gated remote-arg heuristic (+25) floors at Suspicious. Before→after table + live 114-entry zero-diff FP check in `docs/checks/2026-09-24-flolbin-1-fix.md`.
